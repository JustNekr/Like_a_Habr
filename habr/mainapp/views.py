...
def get_top10_users():
    top10_users = User.objects.annotate(total_articles_likes=Sum('articles__likes')).annotate(
        total_comments_likes=Sum('comments__likes')).annotate(total_user_likes=Count('likes')).annotate(
        total_rating=F('total_articles_likes') + F('total_comments_likes') + F("total_user_likes")).order_by(
        '-total_rating')[:10]
    return top10_users
...
class ArticlesView(ListView):
    ...
    def get_context_data(self):
        x = super(ArticlesView, self).get_context_data()
        print()
        return x
...
class ArticleView(DetailView):
    ...
    def get_object(self, queryset=None):
        _article = super(ArticleView, self).get_object()
        if _article and _article.is_banned:
            _article.title = 'Banned article'
            _article.article_text = 'Banned article'
            return _article
        else:
            return _article
